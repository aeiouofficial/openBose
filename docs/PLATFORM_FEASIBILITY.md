...
- API 33 introduced public `BluetoothCodecStatus`, but the status object's existence does not guarantee every device exposes the necessary status intents/API to an ordinary third-party app. Treat the user's Android Developer Options and explicit HCI snoop capture as authoritative diagnostic fallbacks.
