...
4. In a terminal **after** dot-sourcing `D:\openBose\scripts\workspace-env.ps1`, **create the dated capture directory first**: `$session = Join-Path 'D:\openBose\captures\private' ((Get-Date -Format 'yyyy-MM-dd') + '\session-01'); New-Item -ItemType Directory -Path $session -Force | Out-Null`. Then run `adb bugreport (Join-Path $session 'android-bugreport.zip')`. If ADB writes multiple files or a directory, keep all of them under the same private session; do not use the default downloads or temp location.
